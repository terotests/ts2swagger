...
function bootstrap(app: any, server: serverFactory) {
  ...
  app.get("/sometest/v1/ping/:message/", async function(
    req: TRequest,
    res: TResponse
  ) {
    try {
  ...
  app.get("/sometest/v1/hello/:name/", async function(
    req: TRequest,
    res: TResponse
  ) {
    try {
  ...
  app.get("/sometest/v1/getDevice/:id/", async function(
    req: TRequest,
    res: TResponse
  ) {
    try {
      ...
      const id: number | null =
        !isNaN(maybe_id) && Number.isInteger(maybe_id) && maybe_id >= 0
          ? maybe_id
          : null;
      if (id === null) throw { statusCode: 422 };
      const yesno: any =
        req.query.yesno === "true"
          ? true
  ...
  app.get("/sometest/v1/getDeviceSecond/", async function(
    req: TRequest,
    res: TResponse
  ) {
    try {
      ...
      if (typeof req.query.what !== "string") throw { statusCode: 422 };
      res.json(
        await server(req, res).getDeviceSecond(id, yesno, req.query.what)
      );
    } catch (e) {
      res.status(e.statusCode || 400);
      res.json(e);
  ...
  app.post("/sometest/v1/upload/", async function(
    req: TRequest,
    res: TResponse
  ) {
    try {
  ...
  app.get("/sometest/v1/testAnyResp/:value/", async function(
    req: TRequest,
    res: TResponse
  ) {
    try {
  ...
  app.get("/sometest/v1/recursiveTest/", async function(
    req: TRequest,
    res: TResponse
  ) {
    try {
